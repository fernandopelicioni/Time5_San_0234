...
begin
             ...
             if (VDiaFeriado >= VInicioPeriodo) and (VDiaFeriado <= VFimPeriodo) then
                   begin
MessageBox(0, 'Esta tarefa deve ser realizada apenas depois que o movimento deste dia estiver fechado,'+#13+#10+''+#13+#10+'Lembre-se disto antes de confirmar a proxima tela!', 'Atenção', MB_ICONWARNING or MB_OK);
                          if (MessageDlg('Todos os lançamentos Normais deste dia, serão convertidos para FERIADO!'+#13+#10+''+#13+#10+'Você confirma este procedimento?', mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
                               begin
                                    With Q do
                                          begin
                                                Close;
                                                ...
                                                MessageBox(0, 'Não se esqueça!'+#13+#10+''+#13+#10+'Caso voce realize um novo lançamento ou alteração no lançamento deste dia, será necessario realizar este procedimento novamente!', 'Relembre', MB_ICONASTERISK or MB_OK);
                                          end;
                               end;
