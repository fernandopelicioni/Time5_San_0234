...
begin
        if (MessageDlg('Confirma Exclusão do Registro Abaixo: '+#13+ DtProDisp.DataSet.FieldByName('Data').AsString + ' - Evento: '+ DtProDisp.DataSet.FieldByName('Evento').AsString, mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
            begin
 ...
 screen.Cursor:=crHourGlass;
                     With QrPro do
                            begin
                                    ...
                                    if  Eof  then
                                           begin
                                                  MessageBox(0, 'Registro não encontrado!'+#13+#10+''+#13+#10+'Certifique se de que digitou corretamente ou se esse registro não pertence a outra Coligada!', 'Não Encontrado', MB_ICONERROR or MB_OK);
                                                  Edit1.Text:='';
