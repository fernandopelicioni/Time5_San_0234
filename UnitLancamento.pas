                   ...
                   begin
                          ...
                          d:=DayOfWeek(VDataLancamento);
                           case D of
                               ...
                               3: EditSemana.Caption:='Terça-Feira';
                               4: EditSemana.Caption:='Quarta-Feira';
                               5: EditSemana.Caption:='Quinta-Feira';
                               6: EditSemana.Caption:='Sexta-Feira';
                               7: EditSemana.Caption:='Sábado';

                           end;

                          Executa.Click;
                          {// Aqui preciso colocar uma pergunta pra saber se o dia é habilitado para lancamento...
                                  ...
                                  if DtPro.DataSet.FieldByName('IDMOV').asstring <> '' THEN
                                        BEGIN
                                              ...
                                              if (Evento = 1) or (Evento = 13)  then
                                                      begin
//                                                     --       MessageBeep(32);
                                                           if (MessageDlg('Caro Usuário,                                                  '+#13+#10+'Existem Lançamentos deste colaborador na data, Deseja Alterar?'+#13+#10+'Sim = Alterar!'+#13+#10+'Não = Novo Lançamento!', mtConfirmation, [mbYes, mbNo], 0) in [mrYes]) then
                                                                  begin // Escolha Sim
                                                                          // Time.lnacamento não aceitara edit caso o evento não seja Normal ou Feriado, nesse caso, apenas informará
                                                                          PreencheEdit(nil);
                                                                          Pai.TipoEntrada:=1; // avisa sql pai que é uma alteração UPDATE PTO_JORNADA
                                                                          EditHI1.SetFocus;
                                                                   end
                                                                   else
                                                                   begin  // Escolha Nao
                                                                       BtNovo.Click;
//                                                                           if DtPro.DataSet.FieldByName('copiar').AsString = '1' then // CASO NÃO EXISTA, E UTILIZAR HERANÇA DE HORARIO PADRAO FAÇA COMO ABAIXO
//                                                                                  begin
//                                                                                       Pai.TipoEntrada:=0; // avisa o sql q é novo
...
//                                                                                  EditHI1.SetFocus;
                                                      ...
                                                      begin
//                                                             MessageBeep(0);
                                                            MessageBox(0, 'Caro usuário,'+#13+#10+'Pelas regas de negócios, na tela de lançamento não será possível alterar eventos extra-ordinários;'+#13+#10+''+#13+#10+'Verifique no módulo de Eventos em Movimentos!', 'Outros Eventos', MB_ICONINFORMATION or MB_OK);
                                                            BtNovo.Click;
                                                            //abort;
                                                      end;
                                        END
                                        ...
                                        begin
                                                   if DtPro.DataSet.FieldByName('copiar').AsString = '1' then // CASO NÃO EXISTA, E UTILIZAR HERANÇA DE HORARIO PADRAO FAÇA COMO ABAIXO
                                                          begin
                                                               Pai.TipoEntrada:=0;   // avisa a sql que é um novo registro
//                                                               Showmessage(DtPro.DataSet.FieldByName('Tfu_Ini').AsString);
//                                                               EditHI1.Text :=DtPro.DataSet.FieldByName('Tfu_Ini').AsString;
//                                                               EditHf1.Text   :=DtPro.DataSet.FieldByName('Tfu_Fim').AsString;
                                                          end;
//                                                   if DtPro.DataSet.FieldByName('Tfu_Itp').AsString <> '' then
                                                   Sequencial:=1; // Esse variavel indica que é o primeiro lancamento para este colaborador nesta data, isso permitira lancar evento como folga
                                                   EditHI1.SetFocus;
                                              end;
                                 end
                                 else
...
//                                                   MessageBeep(16);
                                                   MessageBox(0, 'Caro usuário,'+#13+#10+'Este código ou chapa do Colaborador não é valido, verifique se o colaborador está ativo, se essa função registra ponto e tente novamente!', 'Xiiiiiiiiiiiii, Algo esta errado!', MB_ICONERROR or MB_OK);
                                                   EditChapa.Text:='';
                                                   EditChapa.SetFocus;
                                 End;
                      end;
...
begin
          ...
          LabelUsuario.caption:='Usuário : ' + Pai.Qrusuarios.FieldByName('Usu_Login').AsString;
          DtPeriodo.DataSet.Open;
          label3.Caption:=DtPeriodo.DataSet.FieldByName('Tpe_Des').AsString;
          Label19.Caption:='Time - Automoção de Ponto - versão : ' + formIniciasistema.editversao.caption;
          label2.Caption:=DtPeriodo.DataSet.FieldByName('Tpe_Inicio').AsString + ' até ' +DtPeriodo.DataSet.FieldByName('Tpe_Fim').AsString;
...
begin
         if (DtIni.Date > StrToDate('15/03/2019'))   then
               begin
                     showmessage('Sistema está configurado para operar apenas a partir de 02/03/2019, devido a lançamentos anteriores');
                     Abort;
               end;
         ...
         if (DtIni.Date >= Vinicio) and (VFim >= DtFim.Date) and (DtFim.Date >= DtIni.Date)  then
                    ...
                    begin
                           MessageBox(0, 'Caro usuário,'+#13+#10+'As datas informadas parecem estar fora do periodo ativo, verifique!', 'Atenção com períodos', MB_ICONSTOP or MB_OK);
                                        ...
                                        //   memo1.Text:=Qrtemp.SQL.text;
                                           ...
                                           if FormImportaEscala.Qrtemp.eof then
                                                 begin
                                                    ...
                                                    if (MessageDlg(''+#13+#10+'Achei os dados referentes a esta identificação e estou pronto para salvar no banco de dados!'+#13+#10+''+#13+#10+'Você confirma essa transação?', mtConfirmation, [mbYes, mbNo], 0) = mrYes) then
                                                           begin
                                                                 Pai.InserirMov1Click(nil);
...
begin
      if (key = Vk_ADD)  then
              ...
              // lançar folga
                      if sequencial = 1 then
                            begin
                                   if (MessageDlg('Você confirma o lançamento para este colaborador?'+#13+#10+'Evento : FOLGA', mtConfirmation, [mbYes, mbNo], 0) =  Mryes) then
                                          begin
                                                folgar.Click;
                                          end;
                                           BtNovo.click;
      ...
      if (key = VK_DECIMAL)  then
              ...
              // lançar folga
                      if sequencial = 1 then
                            begin
                                   if (MessageDlg('Você confirma o lançamento para este colaborador?'+#13+#10+'**************************'+#13+#10+'  EVENTO : FALTA', mtConfirmation, [mbYes, mbNo], 0) =  Mryes) then
                                          begin
                                                Faltar(nil);
                                          end;
                                           BtNovo.click;
      ...
      if (key = VK_f12)  then
              ...
              // lançar folga
                      if sequencial = 1 then
                            begin
                                   if (MessageDlg('Confirme se este colaborador já entregou a documentação no Depto Médico'+#13+#10+'Confirma a inserção ? '+#13+#10+'  EVENTO : 4 - Atestado', mtConfirmation, [mbYes, mbNo], 0) =  Mryes) then
                                          begin
                                                Atestadar;
                                          end;
                                           BtNovo.click;
      ...
      if (key = VK_f11)  then
              ...
              // lançar folga
                      if sequencial = 1 then
                            begin
                                   if (MessageDlg('Confirme ABONAR DIA de colaborador'+#13+#10+'Confirma a inserção ? '+#13+#10+'  EVENTO : 8 - Abonar', mtConfirmation, [mbYes, mbNo], 0) =  Mryes) then
                                          begin
                                                Abonar;
                                          end;
                                           BtNovo.click;
              ...
              begin
                    ...
                    if EditHi2.Text='00:00' then
                       ...
                       begin
                            ...
                            begin
                                  MessageBox(0, 'Corrija intervalo mínimo,'+#13+#10+'Esse intervalo não atingiu o pré requisito mínimo de 15 minutos ', 'Irregularidade...', MB_ICONSTOP or MB_OK);
                                  EditHf1.SetFocus;
                            end;
                       end;
              end;
                            ...
                            with QrVeiculo do
                                begin
                                    ...
                                    if not eof then
                                          ...
                                          begin
                                                close;
                                                MessageBox(0, 'Caro Usuário,'+#13+#10+'Não encontrei este veículo no Banco de Dados, verifique e tente de novo...', 'Opssssssss, algo errado...', MB_ICONSTOP or MB_OK);
                                                Editvei1.Text:='';
                                                Editvei1.SetFocus;
                                          end;
                                end;
...
begin
        ...
        // pesquisa se há algo lancado no campo veiculo
end;
...
///                 // Insere um atestado a partir da tela de lançamentos
                   // Pega Codigo do PtoMov.. Depois preenche os dados para montar o SQL
                     Pai.ENT1     :=0; Pai.SAI1:=0; Pai.TOT1:=0; Pai.VEI1:='0';
                     Pai.ENT2     :=0; Pai.SAI2:=0; Pai.TOT2:=0; Pai.VEI2:='0';
                     Pai.ENT3     :=0; Pai.SAI3:=0; Pai.TOT3:=0; Pai.VEI3:='0';
